...
import Person from "../components/PersonSvg";
...
const RaceTrack = () => {
  ...
  return (
    <div className="bg-[#afa39e] h-screen w-full flex flex-col gap-5 items-center justify-center">
      ...
      <p>{count} Secs</p>
      {data.map((item, index) => (
        <div key={index} className="w-[60%] flex flex-col justify-center relative">
          <label
            htmlFor={`range-${index}`}
          ...
          <Person progress={`${(item.speed * count * 0.2778 * 100) / 400}%`} />
          <input
            id={`range-${index}`}
            type="range"
