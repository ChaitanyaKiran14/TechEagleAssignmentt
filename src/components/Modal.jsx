...
const Modal = ({ handleModal, handleCount }) => {
  ...
  const newData = data
    .map((item) => {
      ...
      if (!isNaN(startTime) && !isNaN(speed) && speed !== 0) {
        const endTime = (startTime + (400 / speed)).toFixed(2);
        return {
          ...item,
          endTime: endTime,
        };
      } else {
        return item; // Return the original item if startTime or speed is not a valid number
      }
    })
